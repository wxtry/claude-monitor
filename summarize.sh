...
SYSTEM_PROMPT="You will receive context from a coding session: project name, working directory, user prompts, and optionally a 'Claude Session Title' (auto-generated by Claude). If a Claude Session Title is provided, treat it as a HIGH-WEIGHT reference — it often accurately captures the session's purpose. You should refine or improve it based on the user prompts, but stay close to its meaning unless the prompts clearly indicate a different activity. Focus on what the user is ACTUALLY DOING based on their prompts — not what the project name suggests. The user may work on tasks unrelated to the project directory. Write a concise title (4-8 words) ${LANG_INSTRUCTION} summarizing the user's main activity. Only fall back to the project name if both the prompts and Claude Session Title are too vague. Output ONLY the title, nothing else."
